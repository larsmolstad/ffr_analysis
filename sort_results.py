...
def simplity_df(df):
    #    todrop = set(df.columns).intersection(['name', 'side', 'vehicle_pos', 'vehicle_x',
        ...
        for i, key in enumerate(keys): #enumerate(result_dict.keys()):
            ...
            for j in range(len(r)):
                ...
                if compound != 'utm_coordinates':
                    ...
                    w.write(j+1, 2*i, x)
                w.write(j+1, 2*i+1, y)# float(r[j]['slopes'][compound]))
        try:
            workbook.save(name)
        except IOError:
            tkMessageBox.showinfo("Close the old xls-file, then press ok...........")
        try:
            workbook.save(name)
        except IOError:
...
if __name__ == '__main__':
    unsorted_results, res, resdict = get_results_from_slope_file(G.slope_file, start=-20)
...
def plot_some_regressions(resdict, nr, plots, n, m):
    import my_plotter as mp, get_data, find_regressions
    import math
    plt.clf()
    for i in plots:
        r = resdict[i][nr]#todo...
        a = get_data.get_file_data(os.path.join(resdir.raw_data_path, r['name']))
        subplot(4,4,i)
        reg = find_regressions.find_all_slopes(a, interval=100, co2_guides=True, plotfun=plt.plot)
