...
def get_yr_soup(dato):
    res = requests.get(
        'https://www.yr.no/nb/historikk/tabell/1-60637/Norge/Viken/Ås/Ås?q=' + dato)
...
def get_all_yr_soups(start=(2015, 0o1, 0o1, 12, 0, 0, 0, 0, 0),
    ...
    def t2tstr(t):
        t = time.gmtime(t)
        return "%s-%s-%s" % (t.tm_year, str(t.tm_mon).zfill(2), str(t.tm_mday).zfill(2))
...
def soup2data(soup):
    try:
        tbl= soup.find_all(class_="hourly-table")
